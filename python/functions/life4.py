#!/usr/bin/python


print "------------------------- Life Expectancy v0.99 ---------------------------"
name = raw_input ("What is your name ? : ")
print name,
age = input ("What is your age? : ")
print name,
expect = input("What age do you expect to live to? : ")
answer = input("How much time do you have left? : ")

def lifeexpect(e,a):
     timeleft = e - a
     return timeleft

timeleft = lifeexpect(expect,age)

while answer != timeleft:
     print "Sorry, name", "That is incorrect!"
     answer = input("How much time do you have left? ")


print "Bingo! you will life for ", timeleft, "years"

print name,
print "Based on our calculations, you will live for", expect - age, " more years"
print "Thank you for using Life Expectancy v0.99"



